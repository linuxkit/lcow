...
RUN dd if=/dev/urandom of=file16 bs=128 count=1
RUN dd if=/dev/urandom of=file17 bs=128 count=1
RUN dd if=/dev/urandom of=file18 bs=128 count=1
RUN dd if=/dev/urandom of=file19 bs=128 count=1
RUN dd if=/dev/urandom of=file20 bs=128 count=1
...
COPY --from=build /file16 /
COPY --from=build /file17 /
COPY --from=build /file18 /
COPY --from=build /file19 /
COPY --from=build /file20 /
