# SUMMARY: Create a file with extended ascii characters on the host and check in a container
# LABELS:
# REPEAT:
...
docker container run --platform linux --rm -v  $env:TEST_TMP`:/test alpine:3.7 sh -c "test -f /test/$fileName"
