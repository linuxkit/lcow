# SUMMARY: Create a file with extended ascii characters in the name on a volume mount and check on the host
# LABELS:
# REPEAT:
...
docker run --platform linux --rm -v  $env:TEST_TMP`:/test alpine:3.7 sh -c "touch /test/$fileName"
