# SUMMARY: Create a local volume and re-use it with two containers
# LABELS:
# REPEAT:
...
docker container run --platform linux --rm -v $env:RT_TEST_NAME`:/test alpine:3.7 test -f /test/foobar
if ($lastexitcode -ne 0) {
    $ret = 1
}
